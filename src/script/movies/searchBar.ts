...
export default function searchBar() {
  const searchBarForm = document.querySelector('.movies__header__search') as HTMLFormElement;
  const apiKey = 'e27b9bf02bdfce7f9cd424ca920eea73'; // Your API Key

  searchBarForm.addEventListener('submit', async (event) => {
    event.preventDefault();

    const searchBarInput = document.querySelector('.movies__header__search__input') as HTMLInputElement;
